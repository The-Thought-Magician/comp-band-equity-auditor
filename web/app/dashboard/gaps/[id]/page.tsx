  ...
  if (error) {
    return (
      <div className="space-y-4">
        <Link href="/dashboard/gaps" className="text-sm text-orange-300 hover:text-orange-200">
          ← Back to gap runs
        </Link>
        <Card>
          <CardBody>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-sm font-semibold text-neutral-200">Raw vs adjusted gap by cohort</h2>
            ...
            <Table>
              ...
              <TBody>
                {filtered.map((r) => {
                  ...
                  return (
                    <TR key={r.id}>
                      <TD className="font-medium text-neutral-200">{r.cohort_key}</TD>
                      <TD>
                      ...
                      <TD className="text-right">
                        <div className="flex h-3 w-32 overflow-hidden rounded-full bg-neutral-800">
      ...
      {filtered.some((r) => normalizeDecomp(r.decomposition).length > 0) && (
        <Card>
          <CardHeader>
            <h2 className="text-sm font-semibold text-neutral-200">Decomposition — what drives each gap</h2>
            <p className="mt-0.5 text-xs text-neutral-500">
          ...
          <CardBody className="space-y-5">
            {filtered.map((r) => {
              ...
              return (
                <div key={r.id} className="rounded-lg border border-neutral-800 bg-neutral-950/40 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="text-sm font-medium text-neutral-200">
                      {r.cohort_key} <span className="text-neutral-500">· {r.dimension}</span>
                    </span>
                    ...
                    {factors.map((f, i) => {
                      ...
                      return (
                        <div key={i} className="grid grid-cols-[120px_1fr_60px] items-center gap-3">
                          <span className="truncate text-xs text-neutral-400">{f.factor}</span>
                          <div className="h-2.5 w-full overflow-hidden rounded-full bg-neutral-800">
                            <div
                              className={v < 0 ? 'h-full bg-sky-500/70' : 'h-full bg-orange-500/70'}
                              style={{ width: `${w}%` }}
                            />
                          </div>
                          <span className="text-right text-xs text-neutral-300">{pct(v)}</span>
                        </div>
                      )
                    })}
                  </div>
          ...
          <CardHeader className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-neutral-200">
                ...
                <span className="text-neutral-500">({drillKey.dimension})</span>
              </h2>
              <p className="mt-0.5 text-xs text-neutral-500">Individuals making up this cohort.</p>
            </div>
              ...
              <Table>
                ...
                <TBody>
                  {drillRows.map((e) => (
                    <TR key={e.id}>
                      <TD>
                        <Link href={`/dashboard/datasets/${run.dataset_id}`} className="text-orange-300 hover:text-orange-200">
